...
before(async () => {
  broker.createService(CommandeService);
  await broker.start();
  ...
  try {
    ...
    console.warn("Skipping commande tests: PostgreSQL unavailable.", message);
    dbAvailable = false;
  }
...
describe("commande.ping", () => {
  it("returns pong", async () => {
    const result = await callAction<string>("commande.ping");
    assert.equal(result, "pong");
  });
});

describe("commande.order.create", () => {
  ...
  it("refuses logistique role", async (t) => {
    ...
    await assert.rejects(
      () =>
        callAction("commande.order.create", {
...
describe("commande.order.get and status", () => {
  ...
  it("allows commercial read access", async (t) => {
    ...
    const order = await callAction<{ id: string }>("commande.order.get", {
      accessToken: tokens.commercial,
      orderId: draftOrderId
    });
...
describe("commande.order.validate and reject", () => {
  it("validates a draft order", async (t) => {
    if (skipIfNoDb(t)) return;

    const created = await callAction<{ id: string }>("commande.order.create", {
      accessToken: tokens.commercial,
      clientId,
      siteCode: "SITE-LYO",
    ...
    const validated = await callAction<{ status: string }>("commande.order.validate", {
      accessToken: tokens.commercial,
      orderId: created.id,
  ...
  it("rejects without reason fails validation", async (t) => {
    ...
    await assert.rejects(
      () =>
        callAction("commande.order.reject", {
          accessToken: tokens.commercial,
          orderId: created.id
        }),
  ...
  it("rejects a draft order with reason", async (t) => {
    ...
    const rejected = await callAction<{ status: string }>("commande.order.reject", {
      accessToken: tokens.commercial,
      orderId: created.id,
...
describe("commande.order.priority", () => {
...
describe("commande.order.delayRisk", () => {
...
describe("commande.client.stats and order.history", () => {
...
describe("commande auth edge cases", () => {
  it("refuses expired token", async () => {
    await assert.rejects(
      () =>
        callAction("commande.order.get", {
